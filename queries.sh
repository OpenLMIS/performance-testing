...
SELECT Q1.username, Q1.facilityId, Q1.programId, Q2.programId, 13
	FROM (
		SELECT U.username, R.facilityId, R.programId
		FROM requisitions R, users U, roles RO, role_rights RR, role_assignments RA WHERE
		R.periodId = 13 AND
		R.status  = 'INITIATED' AND
		RR.rightName = 'CREATE_REQUISITION' AND
		RR.roleId = RO.id AND
		RA.roleId = RO.id AND
		RA.userId = U.id AND
		RA.supervisoryNodeId is NULL AND
		U.facilityId = R.facilityId AND
		RA.programId = R.programId AND
		R.programId %2 = 0
		) AS Q1 JOIN (
		SELECT U.username, R.facilityId, R.programId
		FROM requisitions R, users U, roles RO, role_rights RR, role_assignments RA WHERE
		...
		RA.programId = R.programId AND
		R.programId %2 = 1
		) AS Q2 ON Q1.facilityId = Q2.facilityId AND Q1.programId = Q2.programId+1
		ORDER BY Q1.facilityId, Q1.programId, Q2.programId
 ...
 offset=`expr $limit \* $counter`
  ...
  psql -d open_lmis -U postgres -t -A -F"," -c "
    SELECT Q1.username, Q1.programId, Q1.requisitionId, Q2.programId, Q2.requisitionId FROM (
  	SELECT U.username, R.facilityId, R.programId, R.id as requisitionId
  	...
  	ORDER BY R.facilityId, R.programId
  	) AS Q2 ON Q1.facilityId = Q2.facilityId AND Q1.programId = Q2.programId+1 ORDER BY Q1.facilityId, Q1.programId
